...
that for any trajectory instant, the robot is balanced on at least one foot.} Our planner is designed for indoor
...
output of our algorithm is a dynamically balanced walk motion. This result was first
...
planner and  computes a collision-free  statically balanced path  for a robot which can be fixed or sliding 
...
method, and the results have been validated on the HRP-2 robot.

\section{Randomized Motion Planning on Constraint Manifolds}
\label{sec:wb}

FIXME: Avoid the mixup due to the use of task and constraints?
...
\item the goal is also defined implicitly, by additional inverse kinematics tasks.
\end{enumerate}
...
Section~\ref{sec:extension} the adaptation or RRT random extensions to constrained motion planning.
...
\item Call the \textit{constraint solver} (Algorithm~\ref{algo:newton}) on this configuration, with the static
  balance constraints and the goal task.
...
\WHILE{$\|f(q)\| > \varepsilon$ and $i\leq 20$}
...
Fig.~\ref{fig:goal} shows different random configurations enforcing banlance 
...
(434.3656,81.6056) node [above = 0.1cm, color=dg, text width=1.8cm] {\small{Constrained manifold $\mathcal{M}$}};
...
\end{tabular}
\caption {Experimental results on 20 runs: 3 motion planning problems and 2 goal generations. Time is expressed in seconds.}
