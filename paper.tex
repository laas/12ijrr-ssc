...
\textbf{In this paper, we present a planning algorithm that considers
  ...
  quasi-static configuration space \footnote{A quasi-static
    configuration is a configuration where the robot is balanced on
    its feet with zero speed.}, then this first path is approximated
  by a sequence of dynamic walking trajectories. The proof of
  correctness of the algorithm is based on the concept of small-space
  controllability. Such property allows to approximate any \textit{non
    necessarily admissible} path, by a sequence of admissible ones. In
  our context we prove that dynamic walking makes humanoid robots
