...
\begin{proof}[Proof of Theorem \ref{thm:ssc}]
  ...
  there exist $\eta_x > 0$ such that every configuration $q \in B(\tau(x),\eta_x)$ is reachable 
...
feet are separated from a positive distance. Let  $L>0$ be the positive horizontal distance
...
the CoM and of the ZMP. For $\omega >0$, assuming the CoM follows the trajectory
...
increases. Our current ZMP preview controller relies on the cart-table simplified model.
...
are also applied when planning a statically stable path. Note that this is
due to our current implementation of ZMP preview controller, and does not affect
...
Given a statically  stable path $p$,
...
the cart-table model. The stack of tasks applied to
