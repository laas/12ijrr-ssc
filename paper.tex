...
\begin{algorithm}
\caption{\texttt{SolveConstraints}($q$, f, $\varepsilon$): find $q$ such that $f(q) = 0$}
\label{algo:newton}
\begin{algorithmic}
\STATE $i=0$
...
\item \textbf{Call SolveConstraints (Algorithm~\ref{algo:newton}) on
...
$q_{rand}''$  be the result of a call to $\texttt{SolveConstraints}(q_{rand}',f,\epsilon)$.  It is 
the projection
...
\begin{algorithm}[h]
  \caption{ConstrainedExtend($\mathcal{T},q_{near},q_{rand},f,\epsilon$)}
...
ground. \textbf{Fig~\ref{fig:sliding} shows an example of a whole-body
  ...
  are physically unfeasible.They are, however, easier to generate than
