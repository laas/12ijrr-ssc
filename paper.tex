...
\textbf{This section presents an algorithm for constrained motion
  ...
  submanifold $\mathcal{M}$ of $\mathcal{C}$. $\mathcal{M}$ is called
...
\item \textbf{Call the \textit{constraint solver}
  (Algorithm~\ref{algo:newton}) on this configuration, with the
  intersection of the planning and goal manifolds $\mathcal{M} \cap
  \goalmanifold$.}
...
\textbf{Fig.~\ref{fig:goal} shows resulting random configurations
  enforcing balance ($\mathcal{M}$) and reaching ($\goalmanifold$)
